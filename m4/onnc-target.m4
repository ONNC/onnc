...
[dnl
  ...
  AC_ARG_ENABLE([targets],
    [AS_HELP_STRING([--enable-targets],
       ...
       host, vanilla, clang, x86, x86_64, sparc, powerpc, alpha, aarch64, arm,
  ...
  case "$enableval" in
    all) TARGETS_TO_BUILD="Vanilla CLang X86 Sparc PowerPC Alpha AArch64 ARM NvDla Mips Hexagon CellSPU XCore MSP430 SystemZ Blackfin CBackend CppBackend MBlaze PTX"
        ;;
    *)for a_target in `echo $enableval|sed -e 's/,/ /g' ` ; do
        case "$a_target" in
    clang)
        TARGETS_TO_BUILD="CLang $TARGETS_TO_BUILD"
        AC_DEFINE(ENABLE_CLANG_TARGET, 1, [define CLang target])
        ;;
    vanilla)
  ...
  AM_CONDITIONAL([ENABLE_CLANG_TARGET], [ test "${TARGETS_TO_BUILD/CLang}" != "${TARGETS_TO_BUILD}" ])
