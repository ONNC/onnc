...
int HardSigmoidLower::isMe(const ::onnx::Node& pNode) const
{
  if (pNode.kind() == ::onnx::Symbol("HardSigmoid"))
    return kStdLower;
  return kNotMe;
}

ComputeOperator*
HardSigmoidLower::activate(ComputeGraph& pGraph, ::onnx::Node& pNode) const
{
