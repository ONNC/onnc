...
int UpsampleLower::isMe(const xNode& pNode) const
{
...
ComputeOperator*
UpsampleLower::activate(ComputeGraph& pGraph, xNode& pNode) const
{
  ...
  if (!pNode.hasAttribute(xSymbol("scales")))
    return nullptr;

  // create operators
  onnc::Upsample* op = pGraph.addOperator<onnc::Upsample>(
    pNode.fs(xSymbol("scales")));
