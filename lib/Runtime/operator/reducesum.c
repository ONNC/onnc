...
static int32_t con(int32_t* meofarr, int32_t A_ndim, int32_t* A_dims)
{
  int32_t mul = 1, res = 0;
...
static void Reduce(float* A, float* B, int32_t A_ndim, int32_t* A_dims, int32_t axis, int32_t* meofarr, int32_t idx)
{
  if (idx == A_ndim) {
    meofarr[axis]   = 0;
    ...
    Reduce(A, B, A_ndim, A_dims, axis, meofarr, idx + 1);
  } else {
