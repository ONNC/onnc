...
static void forLoop(int32_t axisIndex, int32_t ndim, const int32_t* restrict lowerBound,
                    ...
                    int32_t* outputIndex, int32_t* axisDistance)
{
  ...
  for (int32_t dimIndex = lowerBound[axisIndex]; dimIndex < uperBound[axisIndex]; dimIndex++) {
    forLoop(axisIndex + 1, ndim, lowerBound, uperBound, input, inputIndex + dimIndex * axisDistance[axisIndex], output,
            outputIndex, axisDistance);
  }
}
