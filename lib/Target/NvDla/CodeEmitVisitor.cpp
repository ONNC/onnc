...
void CodeEmitVisitor::visit(const Conv& pOp)
{
  ...
  for(int g = 0; g < group; g++){
    ...
    int kernel_size_minue_one = (input_W_dims[2]-1);
    int max_conv_H = input_X_dims[2] - kernel_size_minue_one;
    ...
    int total_h = input_X_dims[2] - kernel_size_minue_one;
    int split_H = max_conv_H;
			printf ("DBG4: split_H %d, total_h %d, max_conv_H %d, kernel_size_minue_one %d\n", split_H, total_h, max_conv_H, kernel_size_minue_one);
    for(int h = 0, dst_h = 0; h < total_h; h += split_H){
      int pad_top = (h == 0) ? pads[0] : 0;
      if (max_conv_H != total_h) {
        split_H = (((max_conv_H - kernel_size_minue_one + pad_top) / strides[0]) * strides[0]) - pad_top;
      }
      int op_h = h + split_H >= total_h ? total_h - h : split_H;
			printf ("DBG5: split_H %d, op_h %d, pad_top %d, op_h+kernel_size_minue_one %d\n", split_H, op_h, pad_top, op_h + kernel_size_minue_one);
      int pad_bottom = h + split_H >= total_h ? pads[1] : 0;
      int output_h = ((op_h + pad_top + pad_bottom) + (strides[0] - 1)) / strides[0];

      NVDLA_DBG("conv op_h[%d] output_h[%d]\n", op_h + kernel_size_minue_one, output_h);
      NvDlaCubeInfo finfo((is_image_mode)?NVDLA_CUBE_IMAGE:NVDLA_CUBE_FEATURE, input_X_dims[0], input_X_dims[1], op_h + kernel_size_minue_one, input_X_dims[3], ELEMENT_SIZE);
...
#else
      conv_desc->release = op_h + kernel_size_minue_one;
#endif
      ...
      if (pOp.getNumOfInputs() > 2){
        ...
        add_desc->x2_op.alu_type = SDP_ALU_OP_SUM;//FIXME: since x2_op.enable is 0, I don't think it needs any paticular value. This value is from nv_small_alexnet.nvdla.
...
void CodeEmitVisitor::visit(const LRN& pOp)
{
  ...
  if(alpha != 0.0001f || beta != 0.75f || bias != 1.0f){
    ...
    memset(lut_param, 0, sizeof(struct dla_lut_param));
    {
      ...
      float x = 1.0f;
      for(int i = 0; i < 65; i++){
        lut_param->linear_exp_table[i] = __gnu_f2h_ieee(1.0f/pow((bias + (alpha*x/fsize)),beta));//FIXME: bad hack method.
        x *= 2.0f;
      }
    }
...
void CodeEmitVisitor::visit(const Gemm& pOp)
{
  ...
  if(transA == 0 && transB == 1 && alpha == 1.0 && beta == 1.0){
    ...
    NvDlaCubeInfo finfo(NVDLA_CUBE_FEATURE, input_A_dims[0], input_A_dims[1], input_A_dims[2], input_A_dims[3], ELEMENT_SIZE);
    NvDlaCubeInfo winfo(NVDLA_CUBE_WEIGHT, input_B_dims[0], input_B_dims[1], input_B_dims[2], input_B_dims[3], ELEMENT_SIZE);
    NvDlaCubeInfo binfo(NVDLA_CUBE_FEATURE, input_C_dims[0], input_C_dims[1], input_C_dims[2], input_C_dims[3], ELEMENT_SIZE);
    NvDlaCubeInfo oinfo(NVDLA_CUBE_FEATURE, output_Y_dims[0], output_Y_dims[1], output_Y_dims[2], output_Y_dims[3], ELEMENT_SIZE);
...
void CodeEmitVisitor::visit(const Softmax& pOp)
{
  ...
  NvDlaCubeInfo iinfo(NVDLA_CUBE_FEATURE, input_input_dims[0], input_input_dims[1], input_input_dims[2], input_input_dims[3], ELEMENT_SIZE);
  ...
  NvDlaCubeInfo oinfo(NVDLA_CUBE_FEATURE, output_output_dims[0], output_output_dims[1], output_output_dims[2], output_output_dims[3], ELEMENT_SIZE);
...
int CodeEmitVisitor::packWeight(const Tensor *t, int dims[4], int gidx)
{
  ...
  ILoadable::Blob b;
  b.name = blob_name;
  b.size = (dims[0] * dims[1] * dims[2] * dims[3] * ELEMENT_SIZE + WEIGHT_ATOM_CUBE_SIZE - 1) & ~(WEIGHT_ATOM_CUBE_SIZE - 1);
  b.version.major = 0;
  b.version.minor = 0;
...
int CodeEmitVisitor::packBias(const Tensor *t, int dims[4], int gidx)
{
  ...
  std::string blob_name = "tb-" + std::to_string(m_pMeta.m_NumBlobs++);
  NvDlaCubeInfo finfo(NVDLA_CUBE_FEATURE, 1, dims[0], dims[2], dims[3], ELEMENT_SIZE);
