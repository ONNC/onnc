#; RUN : onnx-as lrn.s | onnx2tg -march bm1880 -S -print-module-before-isel -add-dummy-ctable -add-dummy-weight | FileCheck lrn.s
...
#; CHECK-NEXT:     size: 5
#; CHECK-NEXT:     sum_right_shift_width: {{.*}}
...
graph {
  name: "GoogleNet"
  doc_string: ""
  node { input: "data_0" output: "conv2/norm2_1" name: "" op_type: "LRN" attribute { name: "size" i: 5 type: INT } attribute { name: "alpha" f: 0.0001 type: FLOAT } attribute { name: "beta" f: 0.75 type: FLOAT } attribute { name: "bias" f: 1 type: FLOAT } }
  ...
  output { name: "conv2/norm2_1" type { tensor_type { elem_type: FLOAT shape { dim { dim_value: 1 } dim { dim_value: 64 } dim { dim_value: 56 } dim { dim_value: 56 } } } } }
}
