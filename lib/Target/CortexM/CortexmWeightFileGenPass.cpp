...
Pass::ReturnType CortexmHeaderFileGenPass::runOnModule(Module& pModule){
  ...
  for(struct shift_list* now_shift = first_shift ; now_shift != NULL ; now_shift = now_shift -> next){
    num_of_shift++;

    fprintf(weight_file,"#define RIGHT_SHIFT%d %d\n",num_of_shift,now_shift -> shift_value);
  }

  unsigned int num_of_add = 0;
  for(struct add_list* now_add = first_add ; now_add != NULL ; now_add = now_add -> next){
    num_of_add++;
  ...
  for(struct matmul_list* now_matmul = first_matmul ; now_matmul != NULL ; now_matmul = now_matmul -> next){
    num_of_matmul++;
  ...
  for(struct shape_list* now_shape = first_shape ; now_shape != NULL ; now_shape = now_shape -> next){
    num_of_shape++;
  ...
  for(struct weight_list* now_weight = first_weight ; now_weight != NULL ; now_weight = now_weight -> next){
    num_of_weight++;
		//weight
