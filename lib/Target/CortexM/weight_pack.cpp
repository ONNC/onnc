        ...
        for (int w = 0; w < W; w++) {
          for (int n_ofs = 0; n_ofs < n_size; n_ofs++) {
            for (int c = 0; c < C; c++) {
              ...
              int data_ofs = ((n * 8 + n_ofs + N_offset) * C * H * W) + // n = n*16 + n_ofs
                           c * H * W +                                           // c = c + C_offset
                           (h * W) + w;
              *(blob + blob_ofs) = (float)__gnu_f2h_ieee_vanilla(*(data + data_ofs));
            }
          }
        }
      }
			...
			for(int h=0;h<dims[2];h++){//w
				for(int w=0;w<dims[3];w++){//1
					for(int c=0;c<dims[1];c++){//h*w
						output_data[index] = input_data[(k * dims[2] * dims[3] * dims[1]) + w + (c * dims[2] * dims[3]) + (h * dims[3])];
            index++;
					}
				}
			}
			...
			for(int w=0;w<dims[2];w++){//k
				for(int c=0;c<dims[0];c++){//k*h*w
					for(int k=0;k<dims[3];k++){//1
						output_data[index] = 	input_data[(c * dims[2] * dims[1] * dims[3]) + (h * dims[2] * dims[3]) + (w * dims[3]) + k ];
            index++;
          }
				}
			}
