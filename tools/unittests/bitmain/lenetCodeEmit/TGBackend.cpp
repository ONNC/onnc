...
void TGBackend::sendCmdBuf(void *userData, const void *cmdBuf,
                               uint32_t len) {
...
TGBackend::TGBackend() : _bmkernelHandle(nullptr) {
...
TGBackend &TGBackend::lowering(const onnx::ModelProto &model) {
  ...
  for (int i = 0; i < graph.node_size(); i++) {
    const onnx::NodeProto &node = graph.node(i);
    // handler[node.op_type()](node);
    std::cout << "lowering:" << node.op_type() << std::endl;
    if ("Conv" == node.op_type()) {
